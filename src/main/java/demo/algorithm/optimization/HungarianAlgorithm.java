...
public class HungarianAlgorithm {
	...
	public static void printTime(double time) // Formats time output.
	{
		...
		if (days > 0) timeElapsed = Integer.toString(days) + "d:";
		if (hours > 0)
		...
		while (done == false) // main execution loop
		{
			switch (step) {
				...
				case 3:
					...
					break;
				case 4:
					step = hg_step4(step, cost, mask, rowCover, colCover,
							zero_RC);
					break;
				case 5:
	...
	public static int hg_step1(int step, double[][] cost) {
		...
		for (int i = 0; i < cost.length; i++) {
			minval = cost[i][0];
			for (int j = 0; j < cost[i].length; j++) // 1st inner loop finds min
														// val in row.
			{
				if (minval > cost[i][j]) {
					minval = cost[i][j];
			...
			for (int j = 0; j < cost[i].length; j++) // 2nd inner loop subtracts
														// it.
			{
	...
	public static int hg_step3(int step, int[][] mask, int[] colCover) {
		...
		int count = 0;
		for (int j = 0; j < colCover.length; j++) // Check if all columns are
													// covered.
		{
	...
	(double[][] cost, int[] rowCover, int[] colCover, double maxCost) {
		...
		for (int i = 0; i < cost.length; i++) // Now find the smallest uncovered
												// value.
		{
			for (int j = 0; j < cost[i].length; j++) {
	...
	public static void main(String[] args) {
		...
		System.out
				.println("The winning assignment (" + sumType + " sum) is:\n");
		double sum = 0;
		for (int i = 0; i < assignment.length; i++) {
			// <COMMENT> to avoid printing the elements that make up the
			// assignment
