...
/**
 * Shell排序可以理解为插入排序的变种，它充分利用了插入排序的两个特点:  1）当数据规模小的时候非常高效 2）当给定数据已经有序时的时间代价为O(N)
 * 所以，Shell排序每次把数据分成若个小块，来使用插入排序，而且之后在这若个小块排好序的情况下把它们合成大一点的小块，继续使用插入排序，不停的合并小块，
 ...
 * @author Daniel Cheng
 */
public class ShellSorter<E extends Comparable<E>> extends SortObject<E> {
