...
/**
 * 基数排序:基数排序可以说是扩展了的桶式排序， 比如当待排序列在一个很大的范围内，比如0到999999内，那么用桶式排序是很浪费空间的。
 * 而基数排序把每个排序码拆成由d个排序码，比如任何一个6位数（不满六位前面补0）拆成6个排序码， 分别是个位的，十位的，百位的。。。。
 * 排序时，分6次完成，每次按第i个排序码来排。 一般有两种方式: 1) 高位优先(MSD):  从高位到低位依次对序列排序 2） 低位优先(LSD): 
 * 从低位到高位依次对序列排序 计算机一般采用低位优先法（人类一般使用高位优先），但是采用低位优先时要确保排序算法的稳定性。
...
public class RadixSorter {
	...
	public static void main(String[] args) {
		int[] a = { 1, 4, 8, 3, 2, 9, 5, 0, 7, 6, 9, 10, 9, 135, 14, 15, 11,
				33, 999999999, 222222222, 1111111111, 12, 17, 45, 16 };
		USE_LINK = true;
