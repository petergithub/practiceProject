...
public class Practice extends TestBase {
    ...
    public static final List<Integer> SMALL_IDS = Arrays.asList(new Integer[]{506, 508});

    public static void main(String[] args) {
        System.out.println(SMALL_IDS.contains(506));
        System.out.println(SMALL_IDS.contains(508));
        System.out.println(SMALL_IDS.contains(507));
    }

    @Test
    public void ascii2Char() {
        int iAsciiValue = 40; // Currently just the number 9, but we want Tab character
        // Put the tab character into a string
        String strAsciiTab = Character.toString((char) iAsciiValue);
        log.info("strAsciiTab {}", strAsciiTab);
    }
    ...
    @Test
    public void chg() {
        chg("003400430042003000450038003000300030003600460044003000320031003600300032000D000A");
    }

    public String chg(String rmsg) {
        int i = rmsg.length();
        int num = (int) Math.round((double) i / 4.0);
        String text = "";
        for (i = 1; i <= num; i++) {
            //String inputStr = "&H" + StringUtils.mid(rmsg, (i - 1) * 4 + 1, 4);
            String inputStr = StringUtils.mid(rmsg, (i - 1) * 4 + 1, 4);
            Integer dec = Integer.valueOf(convertFromBaseToBase(inputStr, 16, 10));
            log.info("inputStr {} text {} dec {}", inputStr, text, dec);
            text += Character.toString((char) Math.round(dec));
        }
        log.info("text {}", text);
        return text;
    }
...
//        log.info("countryResponse: {}", countryResponse);
    ...
    public void switch1() {
        ...
        final int three = 3;
        @SuppressWarnings("unused") final Integer four = 4;
