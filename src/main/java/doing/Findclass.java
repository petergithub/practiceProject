...
/**
 * The Findclass class scans one or more directory locations for a given java class name. It locates
 * the class file(s) within the given directory structures and contained JAR/zip files. It's use was
 * prompted by so many deployment headaches when class not found errors are thrown or clashes
 * occurred with two classes of the same name but different versions were caught in a class path.
 * Simple use is: findclass &ltclass name to find&gt -classpath &ltdirecotries to search, separated with a
 * ;&gt
 * 
 * @author Rob Gilham
...
public class Findclass {
	 ...
	 * @param classPath The class path to search. Can be a single directory location or multiple
	 *          locations, separated with a semi-colon.
	 */
	public void findClass(String className, String classPath) {
		foundItems.clear();
		if (null == className) throw new NullPointerException("class name can not be null");
		searchClassName = className.toLowerCase().endsWith(CLASS_EXTENTION) ? className.substring(0,
				className.length() - CLASS_EXTENTION.length()).trim() : className.trim();
		ignorePackage = !searchClassName.contains("."); // If package stated in search, then match only
																										// when full package name matches.
		if (isVerboseMode()) {
			System.out.println("Searching for class '" + searchClassName + "' in '" + classPath + "'");
			System.out
					.println("Searching sub-directories: " + Boolean.toString(isSearchSubDirectories()));
	...
	/**
	 * Recursively called method to scan a single location. If the given location is a directory, its
	 * contents are scanned and all sub directories are Recursively passed into this method. All
	 * Archive files are extracted and passed to this method and single files are checked for matching
	 * the search name.
	 * 
	...
	private void scanFile(File f) {
		if (f.isDirectory() && isSearchSubDirectories()) {
			if (isVerboseMode()) System.out.println("Reading directory " + f.getName());
		...
		} else { // Otherwise its a file, so process it
	...
	/**
	 * Flag to control if a classpath locations'sub directories are included in the search. By default
	 * all sub directory trees are scanned. Setting this to false will prevent this, and only scan the
	 * location given in the class path, ignoring the sub directories.
	 * 
	...
	/**
	 * Checks if the Finder will scan just the locations in the classpath or the sub directories of
	 * those locations also.
	 * 
	 * @return true (Default) if the sub directories are to be scanned, false to scan just the
	 *         location.
	 */
	...
	/**
	 * Flag to control the amount of output to the Logger during the search process. Default is False,
	 * to display just the basic information. Setting to true increases the amount of output
	 * 
	...
	/**
	 * Flag that controls the amount of output to the Logger during the search process. Default is
	 * False, to display just the basic information. When true, the amount of output is more detailed
	 * during the search.
	 * 
	...
	/**
	 * Checks if the Finder will ignore the case of the given class file. By default the class name is
	 * case sensitive. Only classes with matching case will be found.
	 * 
	 * @return true (Default) if the case of the class must match the search name. When false, any
	 *         class that just matches the name, regardless of case, will be found.
	 */
	 ...
	 * 
	 * @param ignoreCase when false, any class with a matching name, regardless of case is found,
	...
	/**
	 * Process the given single file to check if it a class that matches the search name. If the given
	 * file is a match, then its name is added to the found classes list.
	 * 
	 * @param f The File to check.
	 */
	private void processClass(File f) {
		if (isVerboseMode()) System.out.println("Checking class file " + f.getName());
		if (isMatchingName(f.getName())) outputFoundFile(f.getName(), f.getParentFile());
	}

	/**
	 * Process the given archive file to check if it contains a class that matches the search name. If
	 * the given file contains a match, then its name is added to the found classes list.
	 * 
	 * @param f The Archive (jar/zip) File to check.
	 */
	private void processZip(File f) throws IOException {
		if (isVerboseMode()) System.out.println("Checking archive file " + f.getName());
		try {
			...
			while (enteries.hasMoreElements()) {
				...
				if (!entry.isDirectory()) {
					...
					if (isMatchingName(name)) outputFoundFile(name, f);
				}
			}
			zip.close();
	...
	/**
	 * Compares the given name with the search class name, according to the rules configured by the
	 * finder
	 * 
	...
	private boolean isMatchingName(String name) {
		if (!name.toLowerCase().endsWith(CLASS_EXTENTION)) return false;
		name = name.substring(0, name.length() - CLASS_EXTENTION.length()).trim();
		if (ignorePackage) {
			int iPos = name.lastIndexOf(".");
			if (iPos >= 0) name = name.substring(iPos + 1);
		} else if (name.length() > searchClassName.length()) // Otherwise, strip off any leading dir
																													// names from name
	...
	public static void main(String[] args) {
		...
		String classPath = arguments.containsArgument(ARG_PATH) ? arguments.getArgument(ARG_PATH)
				: arguments.getArgument(ARG_PATH2);
	...
	public static void showUse() {
		...
		out.println("-" + ARG_PATH
				+ " <class path>\tThe full file path(s) of the location(s) to search.");
