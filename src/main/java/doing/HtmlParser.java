...
public class HtmlParser {
	...
	public static String[] getElementByTagName(String url, String tag) throws ParserException {
		Parser parser = new Parser(url);
	...
	public static String getStringElementByTagName(String url, String tag) throws ParserException {
		Parser parser = new Parser(url);
		NodeFilter filter = new TagNameFilter(tag);
		NodeList list = parser.extractAllNodesThatMatch(filter);
	...
	public static <T extends Tag> void getTagAttribute(String url, Class<T> clazz, String attr)
			throws ParserException {
		Parser parser = new Parser(url);
		NodeFilter filter = new NodeClassFilter(clazz);
		NodeList list = parser.extractAllNodesThatMatch(filter);
	...
	public static void getRegexContent(String url, String regex) throws ParserException {
		...
		NodeList list = parser.extractAllNodesThatMatch(filter);
		for (int i = 0; i < list.size(); i++) {
			System.out.println((i + 1) + ".\t" + list.elementAt(i).toPlainTextString());
		}
	}

	public static String[] getStringContent(String url, String str) throws ParserException {
		Parser parser = new Parser(url);
	...
	public static void getElementByAttr(String url, String attr) throws ParserException {
	...
	public static String getTableInfo(String html, String charset) throws ParserException {
			...
			if (nodeList.elementAt(i) instanceof TableTag) {
				...
				for (int j = 0; j < rows.length; j++) {
					...
					for (int k = 0; k < td.length; k++) {
						if (StringUtils.isBlank(td[k].toPlainTextString().trim())) {
							continue;
						}
	...
	public static void getSpecifiedTagVisitor(String url) throws ParserException {
		Parser parser = new Parser(url);
		NodeVisitor visitor = new NodeVisitor() {
			public void visitTag(Tag tag) {
				if (tag instanceof HeadTag) {
					System.out.println("HeadTag : Tag name is :" + tag.getTagName() + "\nClass is :"
							+ tag.getClass() + "\nText is :" + tag.getText());
				} else if (tag instanceof TitleTag) {
					System.out.println("TitleTag : Tag name is :" + tag.getTagName() + "\nClass is :"
							+ tag.getClass() + "\nText is :" + tag.getText());

				} else if (tag instanceof LinkTag) {
					System.out.println("LinkTag : Tag name is :" + tag.getTagName() + "\nClass is :"
							+ tag.getClass() + "\nText is :" + tag.getText() + "\ngetAttribute is :"
							+ tag.getAttribute("href"));
				} else {
					System.out.println("Other: Tag name is :" + tag.getTagName() + "\nClass is :"
							+ tag.getClass() + "\nText is :" + tag.getText());
				}
	...
	/**
	 * 使用HtmlParser抓去网页内容: 要抓去页面的内容最方便的方法就是使用StringBean. 里面有几个控制页面内容的几个参数. 在后面的代码中会有说明.
	 * Htmlparser包中还有一个示例StringExtractor 里面有个直接得到内容的方法, 其中也是使用了StringBean . 另外直接解析Parser的每个标签也可以的.
	 */
