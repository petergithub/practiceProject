package org.pu.app.book.trainticket.start;
...
import org.pu.app.book.trainticket.util.HttpUtils;
import org.pu.app.book.trainticket.util.JavaUtils;
import org.pu.app.book.trainticket.util.MailUtils;
import org.pu.app.book.trainticket.util.TrainDataUtils;
import org.pu.utils.DateUtils;
import org.pu.utils.Regex;
...
//							for(String train :StringUtils.split(queryBody, "\\n")){
								...
								if(trainInfos[seatIndex].indexOf("--") == -1  && trainInfos[seatIndex].indexOf("无") == -1){
									String[] matchs = Regex.match(trainInfos[16], "getSelected\\('(.*?)'\\)");
...
//									orderParam.put("passenger_1_seat", seatValue);
									...
									while(count>0){
										...
										if(postOrderBody.indexOf("var train_date_str_ = ") != -1){
											...
											String train_date_str_ = Regex.match(postOrderBody, "var train_date_str_ = \"(.*?)\"")[1];
											String station_train_code_ = Regex.match(postOrderBody, "var station_train_code_ = \"(.*?)\"")[1];
											String start_time_str_ = Regex.match(postOrderBody, "var start_time_str_ = \"(.*?)\"")[1];
											String arrive_time_str_ = Regex.match(postOrderBody, "var arrive_time_str_ = \"(.*?)\"")[1];
			//								String lishi_ = Regex.match(postOrderBody, "var lishi_ = \"(.*?)\"")[1];
...
//											System.out.println(body);
											if(body.indexOf("45分钟") != -1){
												...
												MailUtils.sendMail(TrainDataUtils.query_train_date+"_订票成功",
														...
														+DateUtils.dateToString(now, "HH:mm:ss")
												);
			...
			Date now = new Date();
			System.out.println(DateUtils.dateToString(now, "HH:mm:ss")+"--开始搜索："+ queryStrs[0]+"到"+queryStrs[1]);
			thred.start();
		}
	}
