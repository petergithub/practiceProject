...
public class TrainDataUtils {
	...
	static {
		try {
			String station_name_string = FileUtils.readFileToString(new File(
					JavaUtils.classpath + "/station_name_java.js"), "UTF-8");
	...
	static {
		today = DateUtils.dateToString(new Date(), "yyyy-MM-dd");
	}
	 ...
	 * 请求订单参数名称
	 */
	public static String[] queryOrderParamNames = { "station_train_code",
			"lishi", "train_start_time", "trainno", "from_station_telecode",
			"to_station_telecode", "arrive_time", "from_station_name",
			"from_station_telecode_name", "to_station_name",
			"to_station_telecode_name", "ypInfoDetail" };
	/**
	 * 请求订单参数索引
	 */
	public static int[] queryOrderParamIndexs = { 0, 1, 2, 3, 4, 5, 6, 7, 7, 8,
			8, 9 };
	...
	static {
		...
		// 3 是硬卧
		submitOrderParams.put("passengerTickets", "3,1," + name + ",1,"
				+ cardid + "," + mobile + ",Y");
